...
impl Position {
	...
	pub fn passable_neighbors(self,
		entity_map: &EntityMap,
		passable_query: &Query<&Impassable>) -> Vec<Position>{
	...
	pub fn random_passable_neighbor(self,
		entity_map: &EntityMap,
		passable_query: &Query<&Impassable>) -> Option<Position> {
		self.passable_neighbors(entity_map, passable_query).choose(&mut thread_rng()).copied()
	}
}
