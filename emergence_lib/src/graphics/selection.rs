...
pub(super) fn display_tile_interactions(
    ...
    mut terrain_query: Query<(&TilePos, &mut Handle<StandardMaterial>, &mut Visibility)>,
    ...
    if current_selection.is_changed() || hovered_tiles.is_changed() {
        ...
        for (&tile_pos, mut material, mut visibility) in terrain_query.iter_mut() {
            ...
            if let Some(interaction_material) = handles.interaction_materials.get(&interaction) {
                *material = interaction_material.clone_weak();
